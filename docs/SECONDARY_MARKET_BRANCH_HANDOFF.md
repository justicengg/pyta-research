...
#### Frontend Layer 2 / Research Canvas
- `frontend/` 已升级为正式前端工程（Vite + React + TypeScript）
- Research Canvas 当前主线已采用 `Precision Glass × Quantum Topology`
- 默认动效规则已切换为 `state-driven motion`
  - idle 状态更稳
  - running 状态才增强系统感
- Prompt Orb 系统已接入：
  - 位置：命令输入框右上方
  - 默认 variant：`product_logic`
  - 已保留 3 套可切换实验变体：
    - `traditional`
    - `product_logic`
    - `ai_native`
  - 当前资产已改为透明 orb PNG，不再使用整张 mascot 切片
  - dwell 时间当前统一为 `10s`
  - `ai_native` 已被确认是当前最接近产品方向的候选方案
...
frontend/src/components/layout/PromptMascot.tsx   Prompt Orb 浮层组件
frontend/src/components/canvas/EventChips.tsx     事件chips
frontend/src/lib/orb/promptProfiles.ts            Orb variant / mode / copy 配置
frontend/public/orb-playful.png                   Prompt Orb 透明资源（playful）
frontend/public/orb-tsundere.png                  Prompt Orb 透明资源（tsundere）
```
