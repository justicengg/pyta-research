...
def create_app() -> FastAPI:
    ...
    # ── business routers ──────────────────────────────────────────────────────
    from src.api.routers import sandbox, primary, user_settings, sources, market, connectors, upload
    app.include_router(sandbox.router, prefix='/api/v1', tags=['sandbox'])
    app.include_router(primary.router, prefix='/api/v1', tags=['primary'])
